...
from model.Demand import Demand_mod
...
def add():
    ...
    params = "'" + str(request.form["demand_name"]) + "', '" + str(request.form["demand_code"]) + "', '" + \
             ...
             str(request.form["confluence"]) + "', '" + str(request.form["expect_finishtime"]) + ", " + \
             str(request.form["description"]) + "', " + "'1'"
    sql = "insert into t_demand(demand_name, demand_code, domain_code, apply_userid, model_name, status_code, modify_time, dev_users, confluence, expect_finishtime, description, modfiy_code) values(" + params +")"
